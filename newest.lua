...
if not (assert(#opt.sizes == opt.k, "opt.sizes must have size equal to opt.k")) then
	os.exit(-1)
end
...
enc = nn.Sequential()
for i = 1, opt.k do
	enc:add(nn.Linear(opt.sizes[i], opt.sizes[i+1]))
	enc:add(nn.ReLU())
end
...
dec = nn.Sequential()
for i = opt.k, 1, -1 do
	dec:add(nn.Linear(opt.sizes[i], opt.sizes[i-1]))
	dec:add(nn.ReLU())
end
...
tr_loss = nil
tst_loss = nil
encoder, decoder, tr_loss, tst_loss= alternateMin(opt, encoder, decoder, crit, trainData, testData)
