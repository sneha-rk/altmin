	...
	for i = 1, opt.epochs do
		...
		while cur < opt.train_size do
			...
			if j % opt.print_every == 0 then
			   local t, test_loss = test(testDs, model, conjugateModel, criterion, (iter * opt.train_size / opt.test_size) + j)
			   print(string.format("%d, %1.6f", (iter * opt.train_size / opt.test_size) + j, torch.mean(test_loss)))
			end
