...
opt = 
{
	...
	sizes = {3*1024, 500, 3*1024} --Sizes of inputs in various layers.
...
function load_dataset(train_or_test, count)
	...
	if train_or_test == 'train' then
		data = mnist.loadTrainSet(count, {28, 28})
	else
		data = mnist.loadTestSet(count, {28, 28})
	end

	-- vectorize each 2D data point into 1D
	data.data = data.data:reshape(data.data:size(1), 28*28)
